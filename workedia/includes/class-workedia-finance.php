...
class Workedia_Finance {
    ...
    public static function calculate_member_dues($member_id) {
        ...
        $settings = Workedia_Settings::get_finance_settings();
    ...
    public static function get_total_paid($member_id) {
        ...
        $sum = $wpdb->get_var($wpdb->prepare(
            "SELECT SUM(amount) FROM {$wpdb->prefix}workedia_payments WHERE member_id = %d",
            $member_id
        ));
        return (float)$sum;
    }
    ...
    public static function get_payment_history($member_id) {
        ...
        return $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}workedia_payments WHERE member_id = %d ORDER BY payment_date DESC",
            $member_id
        ));
    }
    ...
    public static function record_payment($data) {
        global $wpdb;
        $table = $wpdb->prefix . 'workedia_payments';
        $current_user_id = get_current_user_id();

        // Sequential Invoice Number: [GOV]-YYYY0000X
        $member = Workedia_DB::get_member_by_id($data['member_id']);
        $gov_key = $member->governorate ?? 'generic';
        $prefix = Workedia_Settings::get_governorate_prefix($gov_key);

        $current_year = date('Y');
        $last_seq = (int)get_option('workedia_invoice_sequence_' . $current_year, 0);
        $new_seq = $last_seq + 1;
        update_option('workedia_invoice_sequence_' . $current_year, $new_seq);
        ...
        if ($insert) {
            ...
            if ($data['payment_type'] === 'membership' && !empty($data['target_year'])) {
                ...
                if ($member && intval($data['target_year']) > intval($member->last_paid_membership_year)) {
                    Workedia_DB::update_member($member->id, ['last_paid_membership_year' => intval($data['target_year'])]);
                }
            }
            ...
            if ($data['payment_type'] === 'license' && !empty($data['target_year'])) {
                if ($member && intval($data['target_year']) > intval($member->last_paid_license_year)) {
                    Workedia_DB::update_member($member->id, ['last_paid_license_year' => intval($data['target_year'])]);
                }
            }
            ...
            Workedia_Logger::log('عملية مالية', $log_details);
            ...
            Workedia_DB::add_document([
                ...
                'file_url' => admin_url('admin-ajax.php?action=workedia_print_invoice&payment_id=' . $payment_id),
    ...
    public static function deliver_invoice($payment_id) {
        global $wpdb;
        $payment = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}workedia_payments WHERE id = %d", $payment_id));
        if (!$payment) return;

        $member = Workedia_DB::get_member_by_id($payment->member_id);
        if (!$member || empty($member->email)) return;

        $workedia = Workedia_Settings::get_workedia_info();
        $invoice_url = admin_url('admin-ajax.php?action=workedia_print_invoice&payment_id=' . $payment_id);

        $subject = "فاتورة سداد إلكترونية - " . $workedia['workedia_name'];
        ...
        $message .= $workedia['workedia_name'];
    ...
    public static function get_financial_stats() {
        ...
        $args = array('limit' => -1);
        $members = Workedia_DB::get_members($args);
