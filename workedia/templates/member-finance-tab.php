...
<?php
$dues = Workedia_Finance::calculate_member_dues($member->id);
$history = Workedia_Finance::get_payment_history($member->id);
?>

<div class="workedia-member-finance-tab" dir="rtl">
    ...
    <div class="workedia-card-grid" style="margin-bottom: 30px;">
        <div class="workedia-stat-card" style="border-right: 5px solid #e53e3e;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray); font-weight: 700;">إجمالي المستحق حالياً (Due)</div>
        ...
        <div class="workedia-stat-card" style="border-right: 5px solid #27ae60;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray); font-weight: 700;">إجمالي المدفوعات المسجلة</div>
        ...
        <div class="workedia-stat-card" style="border-right: 5px solid #111F35;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray); font-weight: 700;">المستحق الكلي (Total Owed)</div>
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 30px;">
        <div>
            ...
            <div class="workedia-table-container" style="margin: 0; border: 1px solid #eee; border-radius: 12px; overflow: hidden;">
            ...
            <div id="workedia-payment-section" style="margin-top: 25px; background: #fffaf0; border: 1px solid #feebc8; padding: 20px; border-radius: 12px;">
                ...
                <form id="record-payment-form">
                    ...
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                        <div>
                            <label class="workedia-label" style="font-size:11px;">المبلغ المحصل:</label>
                            <input type="number" name="amount" class="workedia-input" value="<?php echo $dues['balance']; ?>" step="0.01" required>
                        </div>
                        <div>
                            <label class="workedia-label" style="font-size:11px;">تاريخ السداد:</label>
                            <input type="date" name="payment_date" class="workedia-input" value="<?php echo date('Y-m-d'); ?>" required>
                        </div>
                        <div>
                            <label class="workedia-label" style="font-size:11px;">النوع:</label>
                            <select name="payment_type" class="workedia-select">
                        ...
                        <div>
                            <label class="workedia-label" style="font-size:11px;">للسنة (اختياري):</label>
                            <input type="number" name="target_year" class="workedia-input" value="<?php echo date('Y'); ?>">
                        </div>
                        <div style="grid-column: span 2;">
                            <label class="workedia-label" style="font-size:11px;">البيان / تفاصيل العملية:</label>
        ...
        <div>
            <h4 style="border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 15px; font-weight: 800;"><span class="dashicons dashicons-media-spreadsheet" style="color:var(--workedia-primary-color);"></span> سجل العمليات المالية السابقة</h4>
...
function smSelectForPayment(amount, reason) {
    ...
    document.getElementById('workedia-payment-section').scrollIntoView({ behavior: 'smooth' });
}
</script>
