...
<div class="workedia-global-archive" dir="rtl">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h2 style="margin:0; font-weight: 800; color: var(--workedia-dark-color);">الأرشيف الرقمي الشامل</h2>
        <div style="display: flex; gap: 10px;">
            <button onclick="location.reload()" class="workedia-btn workedia-btn-outline" style="width:auto;"><span class="dashicons dashicons-update"></span> تحديث الأرشيف</button>
        </div>
    </div>
    ...
    <div class="workedia-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 30px; border-bottom: 2px solid #edf2f7; padding-bottom: 0;">
        <a href="<?php echo add_query_arg('sub_tab', 'documents'); ?>" class="workedia-tab-btn <?php echo $active_sub_tab == 'documents' ? 'workedia-active' : ''; ?>" style="text-decoration:none;">
            <span class="dashicons dashicons-portfolio" style="vertical-align: middle; margin-left: 5px;"></span> مستندات الأعضاء
        </a>
        <a href="<?php echo add_query_arg('sub_tab', 'issued'); ?>" class="workedia-tab-btn <?php echo $active_sub_tab == 'issued' ? 'workedia-active' : ''; ?>" style="text-decoration:none;">
            <span class="dashicons dashicons-media-spreadsheet" style="vertical-align: middle; margin-left: 5px;"></span> المستندات الصادرة
        </a>
        <a href="<?php echo add_query_arg('sub_tab', 'finance'); ?>" class="workedia-tab-btn <?php echo $active_sub_tab == 'finance' ? 'workedia-active' : ''; ?>" style="text-decoration:none;">
    ...
    <?php if ($active_sub_tab == 'documents'): ?>
        ...
        $query = "SELECT d.*, m.name as member_name, m.national_id as member_nid FROM {$wpdb->prefix}workedia_documents d JOIN {$wpdb->prefix}workedia_members m ON d.member_id = m.id WHERE $where ORDER BY d.created_at DESC";
        ...
        <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 25px;">
            <form method="get" style="display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap;">
                <input type="hidden" name="workedia_tab" value="global-archive">
                <input type="hidden" name="sub_tab" value="documents">
                <div style="flex: 1; min-width: 250px;">
                    <label class="workedia-label">بحث في المستندات:</label>
                    <input type="text" name="doc_search" class="workedia-input" value="<?php echo esc_attr($search); ?>" placeholder="العنوان، الاسم، أو الرقم القومي...">
                </div>
                <div style="width: 150px;">
                    <label class="workedia-label">التصنيف:</label>
                    <select name="doc_category" class="workedia-select">
                ...
                <button type="submit" class="workedia-btn" style="width: auto; height: 42px;">فلترة النتائج</button>
                <a href="<?php echo remove_query_arg(['doc_search', 'doc_category']); ?>" class="workedia-btn workedia-btn-outline" style="width: auto; height: 42px; text-decoration:none; display:flex; align-items:center;">إعادة ضبط</a>
            </form>
        </div>
                    ...
                    <?php else: ?>
                        ...
                        foreach ($documents as $d): ?>
                            <tr>
                                ...
                                <td><span class="workedia-badge workedia-badge-low"><?php echo $catNames[$d->category] ?? $d->category; ?></span></td>
    ...
    <?php elseif ($active_sub_tab == 'issued'): ?>
        <?php
        $generated = Workedia_DB::get_pub_documents();
        ?>
        <div style="background: #fff; padding: 30px; border-radius: 20px; border: 1px solid #e2e8f0;">
            ...
            <div class="workedia-table-container">
                <table class="workedia-table" id="pub-logs-table">
                    <thead>
                        <tr>
                            <th>الرقم المسلسل</th>
                            <th>عنوان المستند</th>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><span class="workedia-badge workedia-badge-low"><?php echo $type_label; ?></span></td>
    ...
    <?php else: ?>
        ...
        if ($search) {
            $where .= $wpdb->prepare(" AND EXISTS (SELECT 1 FROM {$wpdb->prefix}workedia_members m WHERE m.id = p.member_id AND (m.name LIKE %s OR m.national_id LIKE %s))", '%' . $wpdb->esc_like($search) . '%', '%' . $wpdb->esc_like($search) . '%');
        }
        $payments = $wpdb->get_results("SELECT p.*, u.display_name as staff_name FROM {$wpdb->prefix}workedia_payments p LEFT JOIN {$wpdb->base_prefix}users u ON p.created_by = u.ID WHERE $where ORDER BY p.created_at DESC LIMIT 500");
        ?>
        <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 25px;">
            <form method="get" style="display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap;">
                <input type="hidden" name="workedia_tab" value="global-archive">
                ...
                <div style="flex: 1; min-width: 250px;">
                    <label class="workedia-label">بحث في العمليات المالية:</label>
                    <input type="text" name="fin_search" class="workedia-input" value="<?php echo esc_attr($search); ?>" placeholder="اسم العضو أو الرقم القومي...">
                </div>
                <button type="submit" class="workedia-btn" style="width: auto; height: 42px;">تطبيق البحث</button>
                <a href="<?php echo remove_query_arg(['fin_search']); ?>" class="workedia-btn workedia-btn-outline" style="width: auto; height: 42px; text-decoration:none; display:flex; align-items:center;">إعادة ضبط</a>
            </form>
        </div>
        <div class="workedia-table-container">
            <table class="workedia-table">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($payments as $p):
                             $m = Workedia_DB::get_member_by_id($p->member_id);
                        ?>
                            <tr>
...
<div id="workedia-global-viewer-modal" class="workedia-modal-overlay">
    <div class="workedia-modal-content" style="max-width: 950px; height: 90vh; display: flex; flex-direction: column;">
        <div class="workedia-modal-header">
            <h3 id="workedia-global-viewer-title">عرض المستند</h3>
            <div style="display: flex; gap: 10px; align-items: center;">
                <a href="" id="workedia-global-viewer-download" target="_blank" class="workedia-btn" style="width:auto; height:32px; font-size:11px; background:#27ae60; text-decoration:none; display:flex; align-items:center;">تحميل</a>
                <button class="workedia-modal-close" onclick="document.getElementById('workedia-global-viewer-modal').style.display='none'">&times;</button>
            </div>
        </div>
        <div id="workedia-global-viewer-body" style="flex: 1; background: #525659; overflow: hidden; position: relative;"></div>
    </div>
</div>
...
function smGlobalViewDoc(url, title, id) {
    ...
    const isPdf = url.toLowerCase().endsWith('.pdf') || url.includes('action=workedia_print');
    if (isPdf) {
