...
(function(window) {
    ...
    window.smSubmitPayment = function(btn) {
        ...
        formData.append('action', 'workedia_record_payment_ajax');
        formData.append('nonce', '<?php echo wp_create_nonce("workedia_finance_action"); ?>');
    ...
    window.smOpenFinanceModal = function(memberId) {
        const modal = document.getElementById('workedia-finance-member-modal');
        const body = document.getElementById('workedia-finance-modal-body');
        if (!modal || !body) return;
        modal.style.display = 'flex';
        body.innerHTML = '<div style="text-align:center; padding: 40px;">جاري تحميل البيانات...</div>';

        fetch('<?php echo admin_url('admin-ajax.php'); ?>?action=workedia_get_member_finance_html&member_id=' + memberId)
        .then(r => r.json())
        .then(res => {
            if (res.success) {
...
$is_sys_admin = in_array('workedia_system_admin', $roles);
$is_workedia_admin = in_array('workedia_admin', $roles);
$is_workedia_member = in_array('workedia_member', $roles);
$is_member = in_array('workedia_member', $roles);
$is_officer = $is_workedia_admin || $is_workedia_member;

$active_tab = isset($_GET['workedia_tab']) ? sanitize_text_field($_GET['workedia_tab']) : 'summary';
$is_restricted = $is_member || $is_workedia_member;
...
<div class="workedia-admin-dashboard" dir="rtl" style="font-family: 'Rubik', sans-serif; background: <?php echo $appearance['bg_color']; ?>; border: 1px solid var(--workedia-border-color); border-radius: 12px; overflow: hidden; color: <?php echo $appearance['font_color']; ?>; font-size: <?php echo $appearance['font_size']; ?>; font-weight: <?php echo $appearance['font_weight']; ?>; line-height: <?php echo $appearance['line_spacing']; ?>;">
    ...
    <div class="workedia-main-header">
            ...
            <div>
                ...
                <div style="display: inline-flex; flex-direction: column; align-items: center; padding: 5px 15px; background: #f0f4f8; color: #111F35; border-radius: 12px; font-size: 11px; font-weight: 700; margin-top: 6px; border: 1px solid #cbd5e0; line-height: 1.4;">
                    <div>
                        ...
                        elseif ($is_workedia_admin) echo 'مسؤول Workedia';
                        elseif ($is_workedia_member) echo 'عضو Workedia';
        ...
        <div style="display: flex; align-items: center; gap: 20px;">
            ...
            <?php if (current_user_can('workedia_manage_licenses')): ?>
                <div style="display: flex; gap: 10px;">
                    <?php if ($is_sys_admin || $is_admin): ?>
                        <button onclick="window.location.href='<?php echo add_query_arg('workedia_tab', 'global-archive'); ?>&sub_tab=finance'" class="workedia-btn" style="background: #e67e22; height: 38px; font-size: 11px; color: white !important; width: auto;"><span class="dashicons dashicons-portfolio" style="font-size: 16px; margin-top: 4px;"></span> الأرشيف الرقمي</button>
                    <?php endif; ?>
                    <button onclick="window.location.href='<?php echo add_query_arg('workedia_tab', 'practice-licenses'); ?>&action=new'" class="workedia-btn" style="background: #2c3e50; height: 38px; font-size: 11px; color: white !important; width: auto;" title="إصدار تصريح جديد">+ إصدار تصريح</button>
                    <button onclick="window.location.href='<?php echo add_query_arg('workedia_tab', 'facility-licenses'); ?>&action=new'" class="workedia-btn" style="background: #27ae60; height: 38px; font-size: 11px; color: white !important; width: auto;" title="تسجيل منشأة أو مؤسسة">+ تسجيل منشأة</button>
                </div>
            <?php endif; ?>
            ...
            <div style="display: flex; gap: 15px; align-items: center; border-left: 1px solid var(--workedia-border-color); padding-left: 20px;">
                ...
                <div class="workedia-notifications-dropdown" style="position: relative;">
                    <a href="javascript:void(0)" onclick="smToggleNotifications()" class="workedia-header-circle-icon" title="التنبيهات">
                        ...
                        if (current_user_can('workedia_manage_members')) {
            ...
            <div class="workedia-user-dropdown" style="position: relative;">
                <div class="workedia-user-profile-nav" onclick="smToggleUserDropdown()" style="display: flex; align-items: center; gap: 12px; background: white; padding: 6px 12px; border-radius: 50px; border: 1px solid var(--workedia-border-color); cursor: pointer;">
                ...
                <div id="workedia-user-dropdown-menu" style="display: none; position: absolute; top: 110%; left: 0; background: white; border: 1px solid var(--workedia-border-color); border-radius: 8px; width: 260px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 1000; animation: smFadeIn 0.2s ease-out; padding: 10px 0;">
                    <div id="workedia-profile-view">
                        ...
                        <?php if (!$is_member): ?>
                            <a href="javascript:smEditProfile()" class="workedia-dropdown-item"><span class="dashicons dashicons-edit"></span> تعديل البيانات الشخصية</a>
                        <?php endif; ?>
                        <?php if ($is_member): ?>
                            <a href="javascript:smEditProfile()" class="workedia-dropdown-item"><span class="dashicons dashicons-lock"></span> تغيير كلمة المرور</a>
                        <?php endif; ?>
                        <?php if ($is_admin): ?>
                    ...
                    <div id="workedia-profile-edit" style="display: none; padding: 15px;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <button onclick="smSaveProfile()" class="workedia-btn" style="flex: 1; height: 32px; font-size: 11px; padding: 0;">حفظ</button>
    ...
    <div class="workedia-admin-layout" style="display: flex; min-height: 800px;">
        <!-- SIDEBAR -->
        <?php $is_restricted = $is_member || $is_workedia_member; ?>
        <div class="workedia-sidebar" style="width: 280px; flex-shrink: 0; background: <?php echo $appearance['sidebar_bg_color']; ?>; border-left: 1px solid var(--workedia-border-color); padding: 20px 0;">
            <ul style="list-style: none; padding: 0; margin: 0;">
                ...
                <?php if (!$is_restricted && ($is_admin || $is_sys_admin || $is_workedia_admin)): ?>
                    <li class="workedia-sidebar-item <?php echo in_array($active_tab, ['members', 'update-requests', 'membership-requests']) ? 'workedia-active' : ''; ?>">
                        ...
                        <ul class="workedia-sidebar-dropdown" style="display: <?php echo in_array($active_tab, ['members', 'update-requests', 'membership-requests']) ? 'block' : 'none'; ?>;">
                            ...
                            <li><a href="<?php echo add_query_arg('workedia_tab', 'membership-requests'); ?>" class="<?php echo $active_tab == 'membership-requests' ? 'workedia-sub-active' : ''; ?>"><span class="dashicons dashicons-id-alt"></span> طلبات العضوية</a></li>
                ...
                <?php if (!$is_restricted && ($is_admin || $is_sys_admin || $is_workedia_admin)): ?>
                    <li class="workedia-sidebar-item <?php echo $active_tab == 'finance' ? 'workedia-active' : ''; ?>">
                        <a href="<?php echo add_query_arg('workedia_tab', 'finance'); ?>" class="workedia-sidebar-link"><span class="dashicons dashicons-money-alt"></span> المحاسبة والمالية</a>
                    </li>
                    <li class="workedia-sidebar-item <?php echo in_array($active_tab, ['practice-licenses', 'facility-licenses']) ? 'workedia-active' : ''; ?>">
                        <a href="<?php echo add_query_arg('workedia_tab', 'practice-licenses'); ?>" class="workedia-sidebar-link"><span class="dashicons dashicons-id-alt"></span> التراخيص والمنشآت</a>
                        <ul class="workedia-sidebar-dropdown" style="display: <?php echo in_array($active_tab, ['practice-licenses', 'facility-licenses']) ? 'block' : 'none'; ?>;">
                            <li><a href="<?php echo add_query_arg('workedia_tab', 'practice-licenses'); ?>" class="<?php echo $active_tab == 'practice-licenses' ? 'workedia-sub-active' : ''; ?>"><span class="dashicons dashicons-id-alt"></span> <?php echo $labels['tab_practice_licenses']; ?></a></li>
                            <li><a href="<?php echo add_query_arg('workedia_tab', 'facility-licenses'); ?>" class="<?php echo $active_tab == 'facility-licenses' ? 'workedia-sub-active' : ''; ?>"><span class="dashicons dashicons-building"></span> <?php echo $labels['tab_facility_licenses']; ?></a></li>
                        </ul>
                    </li>
                <?php endif; ?>


                <?php if ($is_admin || $is_sys_admin || $is_workedia_admin): ?>
                    <li class="workedia-sidebar-item <?php echo $active_tab == 'printing' ? 'workedia-active' : ''; ?>">
                        <a href="<?php echo add_query_arg('workedia_tab', 'printing'); ?>" class="workedia-sidebar-link"><span class="dashicons dashicons-printer"></span> مركز الطباعة والنشر</a>
                    </li>
                <?php endif; ?>

                <?php if ($is_admin || $is_sys_admin || $is_workedia_admin || $is_workedia_member || $is_member): ?>
                    ...
                    <?php if ($is_admin || $is_sys_admin || $is_workedia_admin): ?>
                        <li class="workedia-sidebar-item <?php echo $active_tab == 'global-archive' ? 'workedia-active' : ''; ?>">
                            <a href="<?php echo add_query_arg('workedia_tab', 'global-archive'); ?>" class="workedia-sidebar-link"><span class="dashicons dashicons-portfolio"></span> الأرشيف الرقمي</a>
                        </li>
                    <?php endif; ?>
                ...
                <?php if ($is_admin || $is_sys_admin || $is_workedia_admin): ?>
                    <li class="workedia-sidebar-item <?php echo $active_tab == 'global-settings' ? 'workedia-active' : ''; ?>">
                        ...
                        <ul class="workedia-sidebar-dropdown" style="display: <?php echo $active_tab == 'global-settings' ? 'block' : 'none'; ?>;">
                            ...
                            <li><a href="<?php echo add_query_arg('workedia_tab', 'global-settings'); ?>&sub=professional" class="<?php echo ($_GET['sub'] ?? '') == 'professional' ? 'workedia-sub-active' : ''; ?>"><span class="dashicons dashicons-businessman"></span> الدرجات والتخصصات</a></li>
                            <li><a href="<?php echo add_query_arg('workedia_tab', 'global-settings'); ?>&sub=finance" class="<?php echo ($_GET['sub'] ?? '') == 'finance' ? 'workedia-sub-active' : ''; ?>"><span class="dashicons dashicons-money-alt"></span> الرسوم والغرامات</a></li>
        ...
        <div class="workedia-main-panel" style="flex: 1; min-width: 0; padding: 40px; background: #fff;">
            ...
            switch ($active_tab) {
                ...
                case 'members':
                    if ($is_admin || current_user_can('workedia_manage_members')) {
                ...
                case 'finance':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-finance.php';
                    }
                    break;

                case 'financial-logs':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-financial-logs.php';
                    }
                    break;

                case 'practice-licenses':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-practice-licenses.php';
                    }
                    break;

                case 'facility-licenses':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-facility-licenses.php';
                    }
                    break;
                ...
                case 'printing':
                    if ($is_admin || current_user_can('workedia_print_reports')) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-publishing-center.php';
                    }
                    break;
                ...
                case 'surveys':
                    ...
                    } elseif ($is_workedia_member || $is_member) {
                ...
                case 'global-archive':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-global-archive.php';
                    }
                    break;
                ...
                case 'advanced-settings':
                    if ($is_admin || $is_sys_admin) {
                        ...
                        <div class="workedia-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee; overflow-x: auto; white-space: nowrap; padding-bottom: 10px;">
                            <button class="workedia-tab-btn <?php echo ($sub == 'alerts') ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('system-alerts-settings', this)">تنبيهات النظام</button>
                            <button class="workedia-tab-btn <?php echo ($sub == 'staff') ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('system-users-settings', this)">إدارة مستخدمي النظام</button>
                            <button class="workedia-tab-btn <?php echo ($sub == 'backup') ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('backup-settings', this)">مركز النسخ الاحتياطي</button>
                            <button class="workedia-tab-btn <?php echo ($sub == 'logs') ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('activity-logs', this)">سجل النشاطات</button>
                        </div>
                        ...
                        <div id="system-alerts-settings" class="workedia-internal-tab" style="display: <?php echo ($sub == 'alerts') ? 'block' : 'none'; ?>;">
                            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:20px; margin-bottom:20px;">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                                    ...
                                    <button onclick="smOpenAddAlertModal()" class="workedia-btn" style="width:auto; padding:8px 20px;">+ إنشاء تنبيه جديد</button>
                                </div>

                                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:10px; margin-bottom:20px;">
                                    <button onclick="smApplyAlertTemplate('payment')" class="workedia-btn workedia-btn-outline" style="font-size:12px;">قالب: تذكير بالسداد</button>
                                    <button onclick="smApplyAlertTemplate('expiry')" class="workedia-btn workedia-btn-outline" style="font-size:12px;">قالب: تنبيه انتهاء العضوية</button>
                                    <button onclick="smApplyAlertTemplate('maintenance')" class="workedia-btn workedia-btn-outline" style="font-size:12px;">قالب: صيانة النظام</button>
                                    <button onclick="smApplyAlertTemplate('docs')" class="workedia-btn workedia-btn-outline" style="font-size:12px;">قالب: تذكير الوثائق</button>
                                    <button onclick="smApplyAlertTemplate('urgent')" class="workedia-btn workedia-btn-outline" style="font-size:12px;">قالب: قرار إداري عاجل</button>
                                </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div style="display:flex; gap:5px;">
                                                            <button onclick='smEditAlert(<?php echo json_encode($al); ?>)' class="workedia-btn workedia-btn-outline" style="padding:4px 10px; font-size:11px;">تعديل</button>
                                                            <button onclick="smDeleteAlert(<?php echo $al->id; ?>)" class="workedia-btn" style="background:#e53e3e; padding:4px 10px; font-size:11px;">حذف</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                ...
                                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:20px;">
                                    ...
                                    <div style="background:white; padding:20px; border-radius:8px; border:1px solid #eee;">
                                        ...
                                        <div style="display:flex; flex-direction:column; gap:10px;">
                                            ...
                                            <div style="display:flex; gap:8px;">
                                                <button onclick="smDeleteGovData()" class="workedia-btn" style="background:#e53e3e; width:auto; font-size:11px;">حذف بيانات المحافظة</button>
                                                ...
                                                <input type="file" id="workedia_gov_merge_file" style="display:none;" onchange="smMergeGovData(this)">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div style="background:#fff5f5; padding:20px; border-radius:8px; border:1px solid #feb2b2; grid-column: 1 / -1;">
                                        ...
                                        <button onclick="smResetSystem()" class="workedia-btn" style="background:#e53e3e; width:auto; font-weight:800;">إعادة تهيئة النظام بالكامل (Reset)</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:15px;">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                                    ...
                                    <div style="display:flex; gap:10px;">
                                        ...
                                        <button onclick="smDeleteAllLogs()" class="workedia-btn" style="background:#e53e3e; width:auto; font-size:12px; padding:5px 15px;">تفريغ السجل</button>
                                    </div>
                                </div>
                                            ...
                                            ?>
                                                <tr style="border-bottom: 1px solid #f1f5f9;">
                                                    ...
                                                    <td style="padding:6px 8px;">
                                                        <div style="display:flex; gap:5px;">
                                                            <button onclick='smViewLogDetails(<?php echo json_encode($log); ?>)' class="workedia-btn workedia-btn-outline" style="padding:2px 8px; font-size:10px;">التفاصيل</button>
                                                            <?php if ($can_rollback): ?>
                                                                <button onclick="smRollbackLog(<?php echo $log->id; ?>)" class="workedia-btn" style="padding:2px 8px; font-size:10px; background:#38a169;">استعادة</button>
                                                            <?php endif; ?>
                                                            <button onclick="smDeleteLog(<?php echo $log->id; ?>)" class="workedia-btn" style="padding:2px 8px; font-size:10px; background:#e53e3e;">حذف</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                ...
                case 'membership-requests':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        include WORKEDIA_PLUGIN_DIR . 'templates/admin-membership-requests.php';
                    }
                    break;
                ...
                case 'global-settings':
                    if ($is_admin || $is_sys_admin || $is_workedia_admin) {
                        ...
                        ?>
                        <div class="workedia-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee; overflow-x: auto; white-space: nowrap; padding-bottom: 10px;">
                            <button class="workedia-tab-btn <?php echo $sub == 'init' ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('workedia-settings', this)">تهيئة النظام</button>
                            <button class="workedia-tab-btn <?php echo $sub == 'professional' ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('professional-settings', this)">الدرجات والتخصصات</button>
                            <button class="workedia-tab-btn <?php echo $sub == 'finance' ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('finance-settings', this)">الرسوم والغرامات</button>
                            <button class="workedia-tab-btn <?php echo $sub == 'notifications' ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('notification-settings', this)">التنبيهات والبريد</button>
                            <button class="workedia-tab-btn <?php echo $sub == 'design' ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('design-settings', this)">التصميم والمظهر</button>
                            <button class="workedia-tab-btn <?php echo $sub == 'pages' ? 'workedia-active' : ''; ?>" onclick="smOpenInternalTab('page-customization', this)">تخصيص الصفحات</button>
                        </div>
                                    ...
                                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-top:15px;">
                                        ...
                                        <div class="workedia-form-group">
                                            ...
                                            <div style="display:flex; gap:10px;">
                                                ...
                                                <button type="button" onclick="smOpenMediaUploader('workedia_authority_logo_url')" class="workedia-btn" style="width:auto; font-size:12px; background:#4a5568;">اختيار</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-top:15px;">
                                        ...
                                        <div class="workedia-form-group" style="grid-column: span 2;">
                                            ...
                                            <div style="display:flex; gap:10px;">
                                                ...
                                                <button type="button" onclick="smOpenMediaUploader('workedia_logo_url')" class="workedia-btn" style="width:auto; font-size:12px; background:#4a5568;">اختيار الشعار</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div id="professional-settings" class="workedia-internal-tab" style="display: <?php echo ($sub == 'professional') ? 'block' : 'none'; ?>;">
                            <form method="post">
                                <?php wp_nonce_field('workedia_admin_action', 'workedia_admin_nonce'); ?>
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:30px;">
                                    <div class="workedia-form-group">
                                        <label class="workedia-label">الدرجات الوظيفية (درجة واحدة في كل سطر):</label>
                                        <textarea name="professional_grades" class="workedia-textarea" rows="8"><?php
                                            foreach (Workedia_Settings::get_professional_grades() as $k => $v) echo "$k|$v\n";
                                        ?></textarea>
                                        <p style="font-size:11px; color:#666; margin-top:5px;">التنسيق: key|Label (مثال: expert|خبير)</p>
                                    </div>
                                    <div class="workedia-form-group">
                                        <label class="workedia-label">التخصصات المهنية (تخصص واحد في كل سطر):</label>
                                        <textarea name="specializations" class="workedia-textarea" rows="8"><?php
                                            foreach (Workedia_Settings::get_specializations() as $k => $v) echo "$k|$v\n";
                                        ?></textarea>
                                        <p style="font-size:11px; color:#666; margin-top:5px;">التنسيق: key|Label (مثال: massage|تدليك رياضي)</p>
                                    </div>
                                </div>
                                <button type="submit" name="workedia_save_professional_options" class="workedia-btn" style="width:auto; margin-top:10px;">حفظ الخيارات المهنية</button>
                            </form>
                        </div>

                        <div id="finance-settings" class="workedia-internal-tab" style="display: <?php echo $sub == 'finance' ? 'block' : 'none'; ?>;">
                            <?php
                            $fin = Workedia_Settings::get_finance_settings();
                            ?>
                                ...
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:25px; margin-top:20px;">
                                    <div style="grid-column: span 2; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                        ...
                                        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:15px;">
                                            <div class="workedia-form-group"><label class="workedia-label">قيد جديد:</label><input type="number" name="membership_new" value="<?php echo $fin['membership_new']; ?>" class="workedia-input"></div>
                                            <div class="workedia-form-group"><label class="workedia-label">تجديد سنوي:</label><input type="number" name="membership_renewal" value="<?php echo $fin['membership_renewal']; ?>" class="workedia-input"></div>
                                            <div class="workedia-form-group"><label class="workedia-label">غرامة تأخير:</label><input type="number" name="membership_penalty" value="<?php echo $fin['membership_penalty']; ?>" class="workedia-input"></div>
                                        </div>
                                    ...
                                    <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                        ...
                                        <div class="workedia-form-group"><label class="workedia-label">إصدار لأول مرة:</label><input type="number" name="license_new" value="<?php echo $fin['license_new']; ?>" class="workedia-input"></div>
                                        <div class="workedia-form-group"><label class="workedia-label">تجديد (كل سنتين):</label><input type="number" name="license_renewal" value="<?php echo $fin['license_renewal']; ?>" class="workedia-input"></div>
                                        <div class="workedia-form-group"><label class="workedia-label">غرامة تأخير سنوية:</label><input type="number" name="license_penalty" value="<?php echo $fin['license_penalty']; ?>" class="workedia-input"></div>
                                    </div>

                                    <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                        <h5 style="margin:0 0 10px 0;">🏢 تراخيص المنشآت (حسب الفئة)</h5>
                                        <div class="workedia-form-group"><label class="workedia-label">الفئة (A):</label><input type="number" name="facility_a" value="<?php echo $fin['facility_a']; ?>" class="workedia-input"></div>
                                        <div class="workedia-form-group"><label class="workedia-label">الفئة (B):</label><input type="number" name="facility_b" value="<?php echo $fin['facility_b']; ?>" class="workedia-input"></div>
                                        <div class="workedia-form-group"><label class="workedia-label">الفئة (C):</label><input type="number" name="facility_c" value="<?php echo $fin['facility_c']; ?>" class="workedia-input"></div>
                                    </div>
                                </div>

                                <button type="submit" name="workedia_save_finance_settings" class="workedia-btn" style="width:auto; margin-top:25px; padding: 0 40px; height: 45px;">حفظ تسعيرة الخدمات</button>
                            </form>
                        </div>
                        ...
                        <div id="page-customization" class="workedia-internal-tab" style="display: <?php echo $sub == 'pages' ? 'block' : 'none'; ?>;">
                            <div style="background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 25px; margin-bottom: 25px;">
                                        ...
                                        <tbody>
                                            <?php foreach(Workedia_DB::get_pages() as $p): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button onclick='smEditPageSettings(<?php echo json_encode($p); ?>)' class="workedia-btn workedia-btn-outline" style="padding: 5px 10px; font-size: 11px;">تعديل التصميم</button>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                                    ...
                                    <button onclick="smOpenAddArticleModal()" class="workedia-btn" style="width: auto;">+ إضافة مقال جديد</button>
                                </div>
                                        ...
                                        <tbody>
                                            ...
                                            <?php else: foreach($articles as $art): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button onclick="smDeleteArticle(<?php echo $art->id; ?>)" class="workedia-btn" style="background: #e53e3e; padding: 5px 10px; font-size: 11px;">حذف</button>
                                                    </td>
                                                </tr>
                                            <?php endforeach; endif; ?>
                                        </tbody>
...
<div id="workedia-finance-member-modal" class="workedia-modal-overlay">
    <div class="workedia-modal-content" style="max-width: 900px;">
        <div class="workedia-modal-header">
            <h3>التفاصيل المالية للعضو</h3>
            <button class="workedia-modal-close" onclick="document.getElementById('workedia-finance-member-modal').style.display='none'">&times;</button>
        </div>
        <div id="workedia-finance-modal-body" style="padding: 20px;">
