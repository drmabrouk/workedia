...
$registry = $wpdb->get_results($wpdb->prepare(
    "SELECT * FROM {$wpdb->prefix}workedia_members WHERE license_number != '' AND (name LIKE %s OR national_id LIKE %s OR license_number LIKE %s) ORDER BY license_expiration_date ASC",
...
<div class="workedia-practice-licenses" dir="rtl">
    ...
    <div class="workedia-card-grid" style="margin-bottom: 30px;">
        <div class="workedia-stat-card">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">إجمالي التراخيص</div>
            <div style="font-size: 2em; font-weight: 900; color: var(--workedia-dark-color);"><?php echo $stats['total']; ?></div>
        </div>
        <div class="workedia-stat-card" style="border-right: 5px solid #27ae60;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">تراخيص سارية</div>
        ...
        <div class="workedia-stat-card" style="border-right: 5px solid #e67e22;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">تنتهي قريباً (30 يوم)</div>
        ...
        <div class="workedia-stat-card" style="border-right: 5px solid #e53e3e;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">تراخيص منتهية</div>
    ...
    <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 25px;">
        <form method="get" style="display: flex; gap: 15px; align-items: flex-end;">
            <input type="hidden" name="workedia_tab" value="practice-licenses">
            <div style="flex: 1;">
                <label class="workedia-label">بحث في سجل التراخيص:</label>
                <input type="text" name="license_search" class="workedia-input" value="<?php echo esc_attr($search); ?>" placeholder="الاسم، الرقم القومي، أو رقم الترخيص...">
            </div>
        ...
        <table class="workedia-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight: 800; color: var(--workedia-primary-color);"><?php echo esc_html($m->license_number); ?></td>
                    ...
                    <td>
                        ...
                        <?php elseif ($is_soon): ?>
                            <span class="workedia-badge workedia-badge-medium">ينتهي قريباً</span>
                        <?php else: ?>
...
<div id="workedia-license-modal" class="workedia-modal-overlay">
    <div class="workedia-modal-content" style="max-width: 600px;">
        <div class="workedia-modal-header">
            ...
            <button class="workedia-modal-close" onclick="document.getElementById('workedia-license-modal').style.display='none'">&times;</button>
        </div>
        <form id="workedia-license-form" style="padding: 20px;">
            <div class="workedia-form-group">
                <label class="workedia-label">اختر العضو:</label>
                <select name="member_id" class="workedia-select" id="license_member_select" required>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 15px;">
                <div class="workedia-form-group">
                    <label class="workedia-label">رقم الترخيص:</label>
                    <input type="text" name="license_number" id="lic_num" class="workedia-input" required>
                </div>
                <div class="workedia-form-group">
                    <label class="workedia-label">تاريخ الإصدار:</label>
                    <input type="date" name="license_issue_date" id="lic_issue" class="workedia-input" value="<?php echo date('Y-m-d'); ?>" required>
                </div>
                <div class="workedia-form-group">
                    <label class="workedia-label">تاريخ الانتهاء:</label>
                    <input type="date" name="license_expiration_date" id="lic_expiry" class="workedia-input" value="<?php echo date('Y-m-d', strtotime('+1 year')); ?>" required>
                </div>
            </div>
            <button type="submit" class="workedia-btn" style="margin-top: 25px;">حفظ بيانات الترخيص</button>
        </form>
    </div>
</div>
...
document.getElementById('workedia-license-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const formData = new FormData(this);
    formData.append('action', 'workedia_update_license_ajax');
