...
<div class="workedia-financial-logs" dir="rtl">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        ...
        <button onclick="location.reload()" class="workedia-btn workedia-btn-outline" style="width:auto;"><span class="dashicons dashicons-update"></span> تحديث السجل</button>
    </div>

    <!-- Filtering Bar -->
    <div style="background: #f1f5f9; padding: 20px; border-radius: 10px; margin-bottom: 25px; border: 1px solid #e2e8f0;">
        <form method="get" style="display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap;">
            <input type="hidden" name="workedia_tab" value="financial-logs">
            <div style="flex: 1; min-width: 200px;">
                <label style="display:block; font-size:12px; margin-bottom:5px; font-weight:700;">بحث عن عضو:</label>
                <input type="text" name="member_search" value="<?php echo esc_attr($search); ?>" class="workedia-input" placeholder="الاسم أو الرقم القومي...">
            </div>
            <div style="width: 80px;">
                <label style="display:block; font-size:12px; margin-bottom:5px; font-weight:700;">اليوم:</label>
                <select name="log_day" class="workedia-select">
            ...
            <div style="width: 120px;">
                ...
                <select name="log_month" class="workedia-select">
                    <option value="">الكل</option>
                    <?php
            ...
            <div style="width: 100px;">
                ...
                <select name="log_year" class="workedia-select">
                    <option value="">الكل</option>
                    <?php for($i=date('Y'); $i>=2020; $i--) echo "<option value='$i' ".selected($year, $i, false).">$i</option>"; ?>
                </select>
            </div>
            <button type="submit" class="workedia-btn" style="width:auto; height:42px; padding: 0 25px;">تصفية النتائج</button>
            <a href="<?php echo remove_query_arg(['log_day', 'log_month', 'log_year', 'member_search']); ?>" class="workedia-btn workedia-btn-outline" style="width:auto; height:42px; text-decoration:none; display:flex; align-items:center;">إعادة ضبط</a>
        </form>
    </div>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($payments as $p):
                        $member = Workedia_DB::get_member_by_id($p->member_id);
                    ?>
                        <tr>
...
function workediaDeleteTransaction(id) {
    ...
    formData.append('action', 'workedia_delete_transaction_ajax');
