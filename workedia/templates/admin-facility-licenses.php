...
$registry = $wpdb->get_results($wpdb->prepare(
    "SELECT * FROM {$wpdb->prefix}workedia_members WHERE facility_number != '' AND (facility_name LIKE %s OR national_id LIKE %s OR facility_number LIKE %s) ORDER BY facility_license_expiration_date ASC",
...
<div class="workedia-facility-licenses" dir="rtl">
    ...
    <div class="workedia-card-grid" style="margin-bottom: 30px;">
        <div class="workedia-stat-card" style="border-right: 5px solid var(--workedia-dark-color);">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">إجمالي المنشآت</div>
            <div style="font-size: 2em; font-weight: 900;"><?php echo $stats['total']; ?></div>
        </div>
        <div class="workedia-stat-card" style="border-right: 5px solid #27ae60;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">فئة A</div>
            <div style="font-size: 2em; font-weight: 900; color: #27ae60;"><?php echo $stats['cat_a']; ?></div>
        </div>
        <div class="workedia-stat-card" style="border-right: 5px solid #3498db;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">فئة B</div>
            <div style="font-size: 2em; font-weight: 900; color: #3498db;"><?php echo $stats['cat_b']; ?></div>
        </div>
        <div class="workedia-stat-card" style="border-right: 5px solid #e53e3e;">
            <div style="font-size: 0.85em; color: var(--workedia-text-gray);">منتهي</div>
    ...
    <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 25px;">
        <form method="get" style="display: flex; gap: 15px; align-items: flex-end;">
            <input type="hidden" name="workedia_tab" value="facility-licenses">
            <div style="flex: 1;">
                <label class="workedia-label">بحث في سجل المنشآت:</label>
                <input type="text" name="facility_search" class="workedia-input" value="<?php echo esc_attr($search); ?>" placeholder="اسم المنشأة، المالك، أو رقم الترخيص...">
            </div>
            <button type="submit" class="workedia-btn" style="width: auto;">بحث</button>
        </form>
    </div>
    ...
    <div class="workedia-table-container">
        <table class="workedia-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div style="font-weight: 700; color: var(--workedia-primary-color);"><?php echo esc_html($m->facility_name); ?></div>
                    ...
                    <td><span class="workedia-badge workedia-badge-low" style="background:#edf2f7; color:#2d3748;">فئة <?php echo esc_html($m->facility_category); ?></span></td>
                    <td><?php echo esc_html($m->facility_license_expiration_date); ?></td>
                    <td>
                        <?php if ($is_expired): ?>
                            <span class="workedia-badge workedia-badge-high">منتهي</span>
                        <?php else: ?>
                            <span class="workedia-badge workedia-badge-low" style="background:#def7ec; color:#03543f;">ساري</span>
                        <?php endif; ?>
                    </td>
                    <td>
...
<!-- Facility Modal -->
<div id="workedia-facility-modal" class="workedia-modal-overlay">
    <div class="workedia-modal-content" style="max-width: 700px;">
        <div class="workedia-modal-header">
            <h3>تسجيل / تجديد بيانات المنشأة</h3>
            <button class="workedia-modal-close" onclick="document.getElementById('workedia-facility-modal').style.display='none'">&times;</button>
        </div>
        <form id="workedia-facility-form" style="padding: 20px;">
            <div class="workedia-form-group">
                <label class="workedia-label">المالك (العضو):</label>
                <select name="member_id" class="workedia-select" id="facility_owner_select" required>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 15px;">
                <div class="workedia-form-group">
                    <label class="workedia-label">اسم المنشأة:</label>
                    <input type="text" name="facility_name" id="fac_name" class="workedia-input" required>
                </div>
                <div class="workedia-form-group">
                    <label class="workedia-label">رقم ترخيص المنشأة:</label>
                    <input type="text" name="facility_number" id="fac_num" class="workedia-input" required>
                </div>
                <div class="workedia-form-group">
                    <label class="workedia-label">فئة المنشأة:</label>
                    <select name="facility_category" id="fac_cat" class="workedia-select">
                ...
                <div class="workedia-form-group">
                    ...
                    <input type="date" name="facility_license_issue_date" id="fac_issue" class="workedia-input" value="<?php echo date('Y-m-d'); ?>" required>
                </div>
                <div class="workedia-form-group">
                    <label class="workedia-label">تاريخ الانتهاء:</label>
                    <input type="date" name="facility_license_expiration_date" id="fac_expiry" class="workedia-input" required>
                </div>
            </div>
            <div class="workedia-form-group" style="margin-top: 15px;">
                <label class="workedia-label">العنوان التفصيلي:</label>
                <textarea name="facility_address" id="fac_addr" class="workedia-input" rows="2"></textarea>
            </div>
            <button type="submit" class="workedia-btn" style="margin-top: 25px;">حفظ بيانات المنشأة</button>
        </form>
    </div>
</div>
...
document.getElementById('workedia-facility-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const formData = new FormData(this);
    formData.append('action', 'workedia_update_facility_ajax');
    formData.append('nonce', '<?php echo wp_create_nonce("workedia_add_member"); ?>');
